...
===================================================== */
public function missingStudents($subjectId, Request $request)
{
    $guru = $request->user();
    if ($guru->role !== 'guru')
        return response()->json(['status'=>false],403);
...
===================================================== */
public function studentAttendanceBySubject($subjectId)
{
    $today = now()->toDateString();
...
| GURU – REPORT MAPEL
===================================================== */
public function attendanceReport($subjectId)
{
    $today = now()->toDateString();
    ...
    return response()->json([
        'status'=>true,
        'summary'=>[
            'hadir'=>Attendance::where('subject_id',$subjectId)->where('tanggal',$today)->where('status','hadir')->count(),
            'terlambat'=>Attendance::where('subject_id',$subjectId)->where('tanggal',$today)->where('status','terlambat')->count(),
        ]
    ]);
}
